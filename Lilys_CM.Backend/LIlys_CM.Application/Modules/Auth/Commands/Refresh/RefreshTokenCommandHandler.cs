...
namespace Lilys_CM.Application.Modules.Auth.Commands.Refresh;
    ...
    : IRequestHandler<RefreshTokenCommand, RefreshTokenCommandDto>
{
    public async Task<RefreshTokenCommandDto> Handle(RefreshTokenCommand request, CancellationToken ct)
    {
        ...
        if (rt is null || rt.ExpiresAtUtc <= nowUtc)
            throw new Lilys_CMConflictException("Refresh token je nevažeci ili je istekao.");
        ...
        {
            throw new Lilys_CMConflictException("Neispravan klijentski otisak.");
        }

        var user = rt.User;
        if (user is null || !user.IsEnabled || user.IsDeleted)
            throw new Lilys_CMConflictException("Korisnicki nalog je nevažeci.");
