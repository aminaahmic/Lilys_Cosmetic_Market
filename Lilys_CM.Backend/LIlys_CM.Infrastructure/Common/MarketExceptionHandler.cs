...
namespace Lilys_CM.Infrastructure.Common;

/// <summary>
/// Global exception handler for unhandled exceptions.
/// Keeps the same ErrorDto format as the previous middleware.
/// </summary>
public sealed class Lilys_CMExceptionHandler(
    ILogger<Lilys_CMExceptionHandler> logger,
...
) : IExceptionHandler
{
    public async ValueTask<bool> TryHandleAsync(HttpContext ctx, Exception ex, CancellationToken ct)
    {
        ...
        {
            Lilys_CMNotFoundException => StatusCodes.Status404NotFound,
            Lilys_CMConflictException or Lilys_CMBusinessRuleException => StatusCodes.Status409Conflict,
    ...
    private static ErrorDto BuildErrorDto(Exception ex, bool isDev, string traceId)
    {
        ...
        switch (ex)
        {
            case Lilys_CMNotFoundException:
            case Lilys_CMConflictException:
            case Lilys_CMBusinessRuleException:
                code = "entity.error";
                message = ex.Message;
                break;
